...
async function bakeImageEdits(html, pageId) {
  ...
  const staleMarkers = []; // localStorage keys whose IDB payload is missing
  for (const k of Object.keys(allEdits)) {
    ...
    if (val === IDB_MARKER) {
      ...
      // Self-heal: a localStorage marker pointing at a missing IDB payload
      // means the binary was wiped (older migration, manual IDB clear, etc).
      // Drop the marker so future loads don't keep waiting on a ghost.
      if (!resolved) staleMarkers.push(k);
    }
  ...
  if (staleMarkers.length) {
    try {
      const cur = JSON.parse(localStorage.getItem('visionoid_credential_edits_v1') || '{}');
      for (const k of staleMarkers) delete cur[k];
      localStorage.setItem('visionoid_credential_edits_v1', JSON.stringify(cur));
      console.log(`[VN bake] page=${pageId}: dropped ${staleMarkers.length} stale IDB markers`, staleMarkers);
    } catch {}
  }
  let out = html;
